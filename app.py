...
title = '<p style="font-family:roboto; color:red; font-size: 48px;">BizcardX </p>'
...
def data_extract(extract):
    ...
    email_pattern = r'\S+@\S+'
    website_pattern = r'[wW]{3}[\s\.]?\w+[-\.\s]?\w+[\.\s]?\w+[\.\s]?\w+'
    phone_pattern = r"(\+?\d{2,3}\s?[-]?\d{3,5}\s?[-]?\d{4,5})"
    
    name_pattern = r'\b[a-zA-Z]+\b' 
    designation_pattern = r'\b[A-Za-z\s]+\b'
    address_pattern = r'\d+\s[A-Za-z\s,]+'
    pincode_pattern = r'\d{3}\s?\d{1,3}'

    name = designation = company = email = website = primary = address = pincode = None
    ...
    info = [name, designation, company, email, website, primary, address, pincode, result]
    return (info)
...
with st.sidebar:
    ...
    selected = option_menu(
        menu_title="Menu", 
        options=["Home","---","Upload", "View or Modify", "---", "About the app"],  
        icons=["house","", "cloud-upload","search",  "", "envelope"],  
...
if selected == 'Home':
    ...
    if uploaded_file != None:
        #function that decodes the image data
        ...
        with col1:
            ...
            #st.table(pd.Series(info, index=['Name', 'Designation', 'Company', 'Email ID', 'Website', 'Primary Contact', 'Address', 'Pincode', 'Other']))
            ...
            ls_m1 = st.text_input('Contact:', info[5])
            
            ls_add = st.text_input('Address:', info[6])
            ls_pin = st.number_input('Pincode:', info[7])
            ...
            if a:
                db.execute(
                    ...
                    "address VARCHAR(255), pincode int, image bytea)")
                query = "INSERT INTO business_cards (name, designation, company, email, website, primary_no,  " \
                      "address, pincode, image) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
                val = (ls_name, ls_desig, ls_Com, ls_mail, ls_url, ls_m1, ls_add, ls_pin, psycopg2.Binary(image))
    ...
    with col1:
        ...
        y = db.fetchall()
        #fetchall to retrieve all the remaining rows from the result set of a database query.
    ...
    if selected_mode == 'View':
        ...
        with col5:
            db.execute(f"select name, designation, company, email, website, primary_no,  "
                         f"address, pincode from business_cards where name = '{selected_contact}'")
            y = db.fetchall()
            st.table(pd.Series(y[0],index=['Name', 'Designation', 'Company', 'Email ID', 'Website', 'Contact', 'Address', 'Pincode'],name='Card Info'))
        ...
        if a:
            query = f"update business_cards set name = %s, designation = %s, company = %s, email = %s, website = %s, " \
                    f"primary_no = %s, address = %s, pincode = %s where name = '{selected_contact}'"
            val = (ls_name, ls_desig, ls_Com, ls_mail, ls_url, ls_m1, ls_add, ls_pin)
            db.execute(query, val)
            conn.commit()
        ...
        st.markdown(
            f'__<p style="text-align:left; font-size: 20px; color: #000000">You are trying to remove {selected_contact} '
